...
% P is the PSO parameter struct.
% Setting P to [] will invoke default parameters (see CRCBPSO).
% N is the number of independent PSO runs. 
% The output is returned in the struct O. 
% The fields of I are:'dataY': The data vector (a uniformly sampled time series).
...
%Independent runs of PSO in parallel. 
...
for lpruns = 1:nRuns   
    ...
    % Normalize estimated qc signal 
    normSigSq = innerprodpsd(estSig,estSig,inParams.samplFreq,inParams.psdVec);
    % Normalization factor 
    normFac = 1/sqrt(normSigSq);
    estSig = normFac*estSig;
    estAmp = innerprodpsd(inParams.dataY,estSig,inParams.samplFreq,inParams.psdVec);
